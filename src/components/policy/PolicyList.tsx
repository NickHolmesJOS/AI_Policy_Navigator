        ...
        {bulkMode ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <button onClick={toggleSelectAll} className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white transition-colors">
                {selectedIds.size === filteredPolicies.length ? <MinusSquare className="w-3.5 h-3.5" /> : <CheckSquare className="w-3.5 h-3.5" />}
                {selectedIds.size === filteredPolicies.length ? "Deselect all" : "Select all"}
              </button>
              <span className="text-xs text-violet-400">{selectedIds.size} selected</span>
            </div>
